...
class DatasetGIST1M(Dataset):
    ...
    def __init__(self):
        ...
        self.basedir = f'{dataset_basedir}gist1M/'

    def get_queries(self):
        return fvecs_read(f"{self.basedir}gist_query.fvecs")

    def get_train(self, maxtrain=None):
        maxtrain = maxtrain if maxtrain is not None else self.nt
        return fvecs_read(f"{self.basedir}gist_learn.fvecs")[:maxtrain]

    def get_database(self):
        return fvecs_read(f"{self.basedir}gist_base.fvecs")

    def get_groundtruth(self, k=None):
        gt = ivecs_read(f"{self.basedir}gist_groundtruth.ivecs")
        if k is not None:
            assert k <= 100
            gt = gt[:, :k]
        return gt
